...
static void SetupVulkan(const char **extensions, uint32_t extensions_count) {
  ...
  {
    std::vector<const char *> extensions_with_portability;
    extensions_with_portability.reserve(extensions_count + 1);
    for (uint32_t i = 0; i < extensions_count; ++i) {
      extensions_with_portability.push_back(extensions[i]);
    }
    extensions_with_portability.push_back(
        VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
    extensions = extensions_with_portability.data();
    extensions_count =
        static_cast<uint32_t>(extensions_with_portability.size());
